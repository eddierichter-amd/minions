...
class OllamaPerformanceTester:
    ...
    def test_model_performance(
        ...
        for i in range(iterations):
            try:
                ...
                # Initialize clients - Updated for Ollama
                ...
                print(f"   Local (Ollama):    {local_time_spent:.3f}s")
    ...
    def run_comprehensive_test(self, iterations: int = 5) -> Dict:
        ...
        print("🚀 Starting comprehensive Ollama performance testing...")
        print(f"📋 Testing {len(self.ollama_models)} models with {len(self.test_prompts)} prompt sizes")
        print(f"🔄 Running {iterations} iterations per test")
        print(f"⏰ Started at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        
        all_results = []
        
        for model_idx, model in enumerate(self.ollama_models):
            print(f"\n🤖 Model {model_idx + 1}/{len(self.ollama_models)}: {model}")
    ...
    def save_results(self, test_results: Dict, filename: str = None):
        ...
        if filename is None:
            ...
            filename = f"ollama_performance_test_{timestamp}.json"
...
def main():
    ...
    try:
        ...
        #print(f"📋 Found {len(available_models)} available Ollama models: {available_models}")
        
        # Use available models or fallback to common ones
        ollama_models = [
            "deepseek-r1:8b"
        ]
        
        # For testing, you can limit to specific models
    ...
    # Initialize tester
    tester = OllamaPerformanceTester(models=ollama_models, openai_api_key=openai_key)
